...
describe("export debts aging report", () => {
  beforeEach(async () => {
    await resetDatabase();
  });
  it("1.1 export debts aging report failed because user is not login yet", async () => {
    const app = await createApp();

    const response = await request(app).get("/v1/debtsAgingReports/export");
    // check status code
  ...
  it("1.2 export debts aging report failed, dont have permission", async () => {
    const app = await createApp();
    ...
    const response = await request(app)
      .post("/v1/debtsAgingReports/export")
  ...
  it("1.3 export debts aging report success", async () => {
    const app = await createApp();
    ...
    const response = await request(app)
      .get("/v1/debtsAgingReports/export")
      .set("Authorization", `Bearer ${responseLogin.body.accessToken}`);
    // check status code
