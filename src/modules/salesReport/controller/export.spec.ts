...
describe("export sales report", () => {
  beforeEach(async () => {
    await resetDatabase();
  });
  it("1.1 export sales report failed because user is not login yet", async () => {
    const app = await createApp();

    const response = await request(app).get("/v1/salesReports/export");
    // check status code
  ...
  it("1.2 export sales report failed, dont have permission", async () => {
    const app = await createApp();
  ...
  it("1.3 export sales report success", async () => {
    const app = await createApp();
