...
describe("export inventory report", () => {
  beforeEach(async () => {
    await resetDatabase();
  });
  it("1.1 export inventory report failed because user is not login yet", async () => {
    const app = await createApp();

    const response = await request(app).get("/v1/inventoryReports/export");
    // check status code
  ...
  it("1.2 export inventory report failed, dont have permission", async () => {
    const app = await createApp();
    ...
    const response = await request(app)
      .post("/v1/inventoryReports/export")
  ...
  it("1.3 export inventory report success", async () => {
    const app = await createApp();
    ...
    const response = await request(app)
      .get("/v1/inventoryReports/export")
      .set("Authorization", `Bearer ${responseLogin.body.accessToken}`);
    // check status code
