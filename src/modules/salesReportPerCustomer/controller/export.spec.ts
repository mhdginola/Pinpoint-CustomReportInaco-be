...
describe("export sales report per customer", () => {
  beforeEach(async () => {
    await resetDatabase();
  });
  it("1.1 export sales report per customer failed because user is not login yet", async () => {
    const app = await createApp();

    const response = await request(app).get("/v1/salesReportsPerCustomers/export");
    // check status code
  ...
  it("1.2 export sales report per customer failed, dont have permission", async () => {
    const app = await createApp();
    ...
    const response = await request(app)
      .post("/v1/salesReportsPerCustomers/export")
  ...
  it("1.3 export sales report per customer success", async () => {
    const app = await createApp();
    ...
    const response = await request(app)
      .get("/v1/salesReportsPerCustomers/export")
      .set("Authorization", `Bearer ${responseLogin.body.accessToken}`);
    // check status code
