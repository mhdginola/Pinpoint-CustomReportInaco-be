...
describe("export purchase report", () => {
  beforeEach(async () => {
    await resetDatabase();
  });
  it("1.1 export purchase report failed because user is not login yet", async () => {
    const app = await createApp();

    const response = await request(app).get("/v1/purchaseReports/export");
    // check status code
  ...
  it("1.2 export purchase report failed, dont have permission", async () => {
    const app = await createApp();
    ...
    const response = await request(app)
      .post("/v1/purchaseReports/export")
  ...
  it("1.3 export purchase report success", async () => {
    const app = await createApp();
